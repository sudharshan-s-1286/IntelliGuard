...
def test_audit_logger_failure(monkeypatch):
    agent = SecurityAgent()
...
def test_detector_failure(monkeypatch):
    ...
    import agents.security_agent.agent
    monkeypatch.setattr(agents.security_agent.agent, "run_all_detectors", mock_run_all)
