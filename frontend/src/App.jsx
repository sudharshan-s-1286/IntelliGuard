...
import { getHealth } from './services/trustApi'
  ...
  useEffect(() => {
    const checkHealth = async () => {
      try {
        ...
        if (data && (data.status === 'online' || data.status === 'SUCCESS' || data.status === 'ok' || data.status === 'healthy' || Object.keys(data).length > 0)) {
          setHealthStatus('online');
          console.log('🟢 Trust Agent Online');
        } else {
          setHealthStatus('offline');
          console.error('🔴 Trust Agent Offline - Unexpected response', data);
        }
      } catch (error) {
        setHealthStatus('offline');
        console.error('🔴 Trust Agent Offline', error);
      }
    };
    checkHealth();
  }, []);
    ...
    <>
      ...
      {healthStatus && (
        <div className={`fixed bottom-4 right-4 z-50 px-4 py-2 rounded-full shadow-lg border text-sm font-medium flex items-center gap-2 ${
          ...
          {healthStatus === 'online' ? '🟢 Trust Agent Online' : '🔴 Trust Agent Offline'}
        </div>
      )}
      <Routes>
