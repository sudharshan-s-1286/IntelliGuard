...
### Agent Purposes:
- **Security Agent**: Detects and mitigates prompt injections, jailbreaks, data exfiltration attempts, and other adversarial attacks against AI models.
- **Privacy Agent**: Ensures PII and sensitive data are redacted, masked, or anonymized before prompts reach external models, adhering to data protection standards.
- **Compliance Agent**: Validates that AI interactions comply with internal corporate policies and external regulations (e.g., GDPR, HIPAA, SOC2).
- **Trust Agent**: Evaluates the reliability, factuality, and lack of bias in AI-generated responses.
...
- **`agents/`**: Contains subdirectories for each agent (e.g., `security_agent`). Each folder is a self-contained domain.
